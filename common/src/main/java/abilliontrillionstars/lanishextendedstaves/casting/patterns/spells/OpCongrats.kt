package abilliontrillionstars.lanishextendedstaves.casting.patterns.spells
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayer) throw MishapBadEntity(
            target,
            Component.translatable("text.lanishextendedstaves.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayer) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            ctx.caster.sendSystemMessage(Component.translatable("text.lanishextendedstaves.congrats", player.displayName));
        }
    }
}
